...
def newsalehook(openseadata, webhook_url):
    ...
    data = {
        "content" : "",
        "username" : "CactusNFT Sales Tracker",
    ...
    embed_fields.append({
            "name": "Sale Price",
            "value": "{} Ξ\n${} USD\n".format(eth_price, round(float(eth_price) * float(eth_usd_conversion), 2))})

    try:
        embed_fields.append({
        "name": "Buyer",
        "value": "[{}]({})".format(openseadata["Buyer"]["user"]["username"], "https://opensea.io/{}".format(openseadata["Buyer"]["address"])),
        "inline": True})
    ...
    try:
        embed_fields.append({
        "name": "Seller",
        "value": "[{}]({})".format(openseadata["Seller"]["user"]["username"], "https://opensea.io/{}".format(openseadata["Seller"]["address"])),
        "inline": True})
    except:
        embed_fields.append({
        "name": "Buyer",
    ...
    thumbnail = {
        "url": openseadata["Project"]["image_url"]
    }
    ...
    footer = {
        "text": "CactusNFT Sales Tracker • {}".format(current_time),
        "icon_url": "https://pbs.twimg.com/profile_images/1412152546618183681/mXJ2GHGv_400x400.jpg"
    }
    ...
    data["embeds"] = [
        {
            "description" : "",
            "title" : "New sale for {}".format(openseadata["Project"]["asset_contract"]["name"] + " - " + openseadata["Project"]["token_id"]),
